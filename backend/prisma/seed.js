...
function randomSymptoms(maxCount) {
...
async function main() {
    ...
    for (const def of PATIENTS) {
        ...
        const alertEntries = entries.filter((e) => e.sys >= 140 || e.dia >= 90 || !e.medTaken);
        const createdAlerts = [];
        for (const ae of alertEntries) {
            ...
            const statusOptions = severity === 'HIGH' ? ['OPEN', 'ACKNOWLEDGED'] : ['OPEN', 'ACKNOWLEDGED', 'RESOLVED'];
            try {
        ...
        const callDefs = [
            { status: 'UPCOMING', offset: -3, type: 'Follow-up', note: 'Scheduled follow-up for elevated BP readings.' },
            { status: 'UPCOMING', offset: -7, type: 'Care Plan Review', note: 'Review medication effectiveness and lifestyle changes.' },
            { status: 'COMPLETED', offset: 5, type: 'Check-in', note: 'Patient confirmed medication adherence. BP trending down.' },
            { status: 'MISSED', offset: 10, type: 'Follow-up', note: 'Patient did not answer. Rescheduling required.' },
        ];
        let callCount = 0;
        ...
        console.log(`[${def.riskTier.padEnd(8)}] ${def.name.padEnd(20)} | ${entries.length} entries | ${highCount}H/${medCount}M alerts | ${escalationCount} escalations | ${totalMessages} messages | OTP: ${def.otp}`);
    }
    console.log('\n' + '═'.repeat(60));
